...
from cartopy.feature import LAKES, OCEAN, NaturalEarthFeature
...
def get_proj_lcc(wps_file):
    ...
    lccproj = ccrs.LambertConformal(central_longitude=ref_lon,
...
def calc_corner_point_latlon(center_lat, center_lon, e_we, e_ns, dx, dy,
                             wpsproj, latlonproj, loc):
    center_x, center_y = wpsproj.transform_point(center_lon, center_lat,
                                                 latlonproj)
    if loc == 'll':
...
def calc_center_point_latlon(corner_lat_parent, corner_lon_parent, dx_parent,
                             dy_parent, e_we, e_ns, dx, dy, i, j, wpsproj,
                             latlonproj):
    corner_x_parent, corner_y_parent = wpsproj.transform_point(
        corner_lon_parent, corner_lat_parent, latlonproj)
    center_x_child = corner_x_parent + dx_parent * i + dx * e_we / 2.0
    center_y_child = corner_y_parent + dy_parent * j + dy * e_ns / 2.0
    center_lon_child, center_lat_child = latlonproj.transform_point(
        center_x_child, center_y_child, wpsproj)
...
def calc_wps_domain_info(wps_file):
    ...
    grid_ratios = get_wps_param_value(wps_file, 'parent_grid_ratio', ndomain,
                                      'int')
    i_parent_start_array = get_wps_param_value(wps_file, 'i_parent_start',
                                               ndomain, 'int')
    j_parent_start_array = get_wps_param_value(wps_file, 'j_parent_start',
                                               ndomain, 'int')
    ...
    if ndomain > 1:
        ...
        e_we = e_we_array
        e_ns = e_ns_array
    corner_lon_full[0, 0], corner_lat_full[0, 0] = calc_corner_point_latlon(
        float(center_lat_full[0]), float(center_lon_full[0]), e_we, e_ns,
        float(dx_full[0]), float(dy_full[0]), wpsproj, latlonproj, 'll')
    corner_lon_full[0, 1], corner_lat_full[0, 1] = calc_corner_point_latlon(
        center_lat_full[0], center_lon_full[0], e_we, e_ns, dx_full[0],
        dy_full[0], wpsproj, latlonproj, 'lr')
    corner_lon_full[0, 2], corner_lat_full[0, 2] = calc_corner_point_latlon(
        center_lat_full[0], center_lon_full[0], e_we, e_ns, dx_full[0],
        dy_full[0], wpsproj, latlonproj, 'ul')
    corner_lon_full[0, 3], corner_lat_full[0, 3] = calc_corner_point_latlon(
        center_lat_full[0], center_lon_full[0], e_we, e_ns, dx_full[0],
        dy_full[0], wpsproj, latlonproj, 'ur')
    ...
    if ndomain > 1:
        for i in np.arange(1, ndomain):
            ...
            center_lon_full[i], center_lat_full[i] = calc_center_point_latlon(
                ...
                dx_full[i - 1], dy_full[i - 1], e_we_array[i], e_ns_array[i],
                dx_full[i], dy_full[i], i_parent_start_array[i],
                j_parent_start_array[i], wpsproj, latlonproj)
            corner_lon_full[i, 0], corner_lat_full[
                i, 0] = calc_corner_point_latlon(center_lat_full[i],
                                                 center_lon_full[i],
                                                 e_we_array[i], e_ns_array[i],
                                                 dx_full[i], dy_full[i],
                                                 wpsproj, latlonproj, 'll')
            corner_lon_full[i, 1], corner_lat_full[
                i, 1] = calc_corner_point_latlon(center_lat_full[i],
                                                 center_lon_full[i],
                                                 e_we_array[i], e_ns_array[i],
                                                 dx_full[i], dy_full[i],
                                                 wpsproj, latlonproj, 'lr')
            corner_lon_full[i, 2], corner_lat_full[
                i, 2] = calc_corner_point_latlon(center_lat_full[i],
                                                 center_lon_full[i],
                                                 e_we_array[i], e_ns_array[i],
                                                 dx_full[i], dy_full[i],
                                                 wpsproj, latlonproj, 'ul')
            corner_lon_full[i, 3], corner_lat_full[
                i, 3] = calc_corner_point_latlon(center_lat_full[i],
                                                 center_lon_full[i],
                                                 e_we_array[i], e_ns_array[i],
                                                 dx_full[i], dy_full[i],
...
def reproject_corners(corner_lons, corner_lats, wpsproj, latlonproj):
    ...
    corner_x[0], corner_y[0] = wpsproj.transform_point(corner_lons[0],
                                                       corner_lats[0],
                                                       latlonproj)
    corner_x[1], corner_y[1] = wpsproj.transform_point(corner_lons[1],
                                                       corner_lats[1],
                                                       latlonproj)
    corner_x[2], corner_y[2] = wpsproj.transform_point(corner_lons[2],
                                                       corner_lats[2],
                                                       latlonproj)
    corner_x[3], corner_y[3] = wpsproj.transform_point(corner_lons[3],
                                                       corner_lats[3],
                                                       latlonproj)
...
def lambert_xticks(ax, ticks, size):
    ...
    xticks, xticklabels = _lambert_ticks(
        ax, ticks, 'bottom', lambda t, n, b: np.vstack((np.zeros(
            n) + t, np.linspace(b[2], b[3], n))).T, lambda xy: xy[0])
...
def lambert_yticks_left(ax, ticks, size):
    ...
    yticks, yticklabels = _lambert_ticks(
        ax, ticks, 'left', lambda t, n, b: np.vstack((np.linspace(
            b[0], b[1], n), np.zeros(n) + t)).T, lambda xy: xy[1])
    ax.yaxis.tick_left()
...
def lambert_yticks_right(ax, ticks, size):
    ...
    # lc = lambda t, n, b: np.vstack((np.linspace(b[0], b[1], n), np.zeros(n) + t)).T
    yticks, yticklabels = _lambert_ticks(
        ax, ticks, 'right', lambda t, n, b: np.vstack((np.linspace(
            b[0], b[1], n), np.zeros(n) + t)).T, lambda xy: xy[0])
    ax.yaxis.tick_right()
    ax.set_yticks(yticks)
    ax.set_yticklabels(
        [ax.yaxis.get_major_formatter()(ytick) for ytick in yticklabels],
...
wpsproj, latlonproj, corner_lat_full, corner_lon_full, length_x, length_y = \
    calc_wps_domain_info(infile)

fig1 = plt.figure(figsize=(10, 8))
ax1 = plt.axes(projection=wpsproj)

ax1.pcolormesh(dem_lons,
               dem_lats,
               dem,
               cmap=plt.cm.terrain,
               vmin=0,
...
# d01
corner_x1, corner_y1 = reproject_corners(corner_lon_full[0, :],
                                         corner_lat_full[0, :], wpsproj,
                                         latlonproj)
ax1.set_xlim(
    [corner_x1[0] - length_x[0] / 15, corner_x1[3] + length_x[0] / 15])
ax1.set_ylim(
    [corner_y1[0] - length_y[0] / 15, corner_y1[3] + length_y[0] / 15])
# ax1.add_feature(OCEAN, edgecolor='k', facecolor='lightblue', zorder=3)

# d01 box
ax1.add_patch(
    mpl.patches.Rectangle((corner_x1[0], corner_y1[0]),
                          length_x[0],
                          length_y[0],
                          fill=None,
                          lw=3,
                          edgecolor='black',
                          zorder=10))
ax1.text(corner_x1[0] + length_x[0] * 0.05,
         corner_y1[0] + length_y[0] * 0.9,
         'D01',
...
# d02 box
corner_x2, corner_y2 = reproject_corners(corner_lon_full[1, :],
                                         corner_lat_full[1, :], wpsproj,
                                         latlonproj)
ax1.add_patch(
    mpl.patches.Rectangle((corner_x2[0], corner_y2[0]),
                          length_x[1],
                          length_y[1],
                          fill=None,
                          lw=3,
                          edgecolor='red',
                          zorder=10))
ax1.text(corner_x2[0] + length_x[1] * 0.05,
         corner_y2[0] + length_y[1] * 0.8,
         'D02',
         fontproperties=font0,
         size=15,
         color='red',
...
xticks = [72, 74, 76, 78, 80, 82, 84, 86, 88, 90, 92]
yticks = [33, 35, 37, 39, 41, 43, 45, 47]
...
ax1.set_title('WRF Domain', size=20)
...
# fig1.savefig('WRF_domain.png', dpi=600)